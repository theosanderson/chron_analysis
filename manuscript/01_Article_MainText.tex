...
We used Chronumental to assign dates to each node in the trees created by UCSC (\Cref{publictree}), and GISAID. Within the first 180 steps of fitting, the algorithm was able to place the median terminal nodes within a day of its position in the metadata. After fitting 90\% of nodes were placed within 3 days of their metadata position (and 99\% within 2 weeks) -- supervision is provided on these dates, so this simply measures the algorithms ability to reconcile date metadata into a tree structure, rather than its ability to predict the dates for nodes where the date is unknown.
