...
p<-ggplot(combo %>% filter(abs(error>10)|rnorm(nrow(combo))>0.99),aes(x=date_proc,y=as_date(predicted_date),color=lineage_new))+geom_point(size=0.1)+scale_color_manual(values=c(tableau_color_pal()(7),"gray")) +theme_bw()+labs(x="Date from metadata",y="Predicted date (Chronumental)",color="Lineage")

ggsave("../manuscript/Figures/anac.png", width=4,height=3)
