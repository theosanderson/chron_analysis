...
ggplot(all %>% filter(name!="metadata_subset_5e-06.tsv",name!="metadata_subset_5e-05.tsv",name!="metadata_subset_0.01.tsv"),aes(x=days_diff,color=factor(gsub("\\.*0+%","%",scales::label_percent()(prop)),levels =levels))) + geom_density(bw=2,n=3000)+coord_cartesian(xlim=c(-60,60))+theme_bw()+scale_color_viridis_d(option="turbo")+labs(color="Metadata retained",x="Prediction error",y="Density")
ggsave("../manuscript/Figures/blinding.pdf",width=3.5,height=3.5)
```
```{r}
